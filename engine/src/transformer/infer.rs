...
use super::{state::{RunStateView, TransformerWeightsView, QTransformerWeightsView, QRunStateView}, Config, Storage};
...
// forward for quantized weights
pub fn q_forward<'a, T: Storage, D: Device<T>>(cfg: &Config, wv: &QTransformerWeightsView<'a, T>, rsv: &mut QRunStateView<'a, T>, token: usize, pos: usize,  device: &D) {
    // let mut cpu_state = RunState::from_config(&cfg); // for debugging
    let dim: usize = cfg.dim;
    let hidden_dim = cfg.hidden_dim;
    let head_size = dim / cfg.n_heads;
